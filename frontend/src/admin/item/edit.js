...
export default function PersistentDrawerLeft() {
  ...
  const session_edit = JSON.parse(window.sessionStorage.getItem("admin_item"));
  //   console.log(session_edit);

  const [name, setName] = useState(session_edit.item_name);

  const [content, setcontent] = useState(session_edit.item_content);
  const [price, setprice] = useState(session_edit.item_price);
  const [stock, setstock] = useState(session_edit.item_stock);
